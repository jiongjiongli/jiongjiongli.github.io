...
Let $X$ be a set and ${\vec{\mathbf {x}}} \in X$ a given point. Then an algorithm, $\mathcal{A}$, with initial point ${\vec{\mathbf {x}}}_0$ is a point-to-set map which generates a sequence $\left \{{\vec{\mathbf {x}}}_t \right \}_{t=0} ^{\infty}$ via the rule
