...
### Global Convergence
Let $X$ be a set and $\overrightarrow {\mathbf {x}_{0}} \in X$ a given point. Then an algorithm, $\mathcal{A}$, with initial point $\overrightarrow {\mathbf {x}_{0}}$ is a point-to-set map which generates a sequence ${\left \lbrace \overrightarrow {\mathbf {x}_{t}} \right \rbrace} _{t=0} ^{\infty}$ via the rule

$$
\overrightarrow{\mathbf {x}_{t + 1}} \in \mathcal{A} \left (\overrightarrow {\mathbf {x}_{t}} \right ), t = 0, 1, \cdots
$$

$\mathcal{A}$ is said to be globally convergent if: For any chosen initial point $\overrightarrow {\mathbf {x}_{0}}$, the sequence ${\left \lbrace \overrightarrow {\mathbf {x}_{t}} \right \rbrace} _{t=0} ^{\infty}$ generated by $\overrightarrow{\mathbf {x}_{t + 1}} \in \mathcal{A} \left (\overrightarrow {\mathbf {x}_{t}} \right )$ (or a subsequence) converges to a point for which a necessary condition of optimality holds.
