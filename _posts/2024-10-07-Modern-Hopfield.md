...
The update rule (2) converges globally: for $\boldsymbol{\xi}^{t+ 1} = \mathcal{A} \left ( \boldsymbol{\xi}^{t} \right )$, the energy $\operatorname {E} \left ( \boldsymbol{\xi}^{t} \right ) \to \operatorname {E} \left ( \boldsymbol{\xi}^{1} \right )$ for $t \to \infty $ and a fixed point $\boldsymbol{\xi}^{1}$.
