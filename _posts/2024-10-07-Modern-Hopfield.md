...
### Global Convergence
Let $X$ be a set and ${\vec{\mathbf {x}}}_{0} \in X$ a given point. Then an algorithm, $\mathcal{A}$, with initial point ${\vec{\mathbf {x}}}_{0}$ is a point-to-set map which generates a

sequence ${\left \lbrace {\vec{\mathbf {x}}}_t \right \rbrace} _{t=0} ^{\infty}$ via the rule
